...
class RentalRepository @Inject constructor(
     ...
     */
    suspend fun createRental(vehicleId: String, plan: String): Result<RentalResponse> =
        runCatching { rentalApi.create(CreateRentalRequest(vehicleId, plan)) }
    ...
    suspend fun uploadPhoto(rentalId: String, side: String, file: File): Result<RentalPhotosState> =
        runCatching {
            ...
            rentalApi.uploadPhoto(rentalId, sidePart, filePart)
        }
     ...
     */
    suspend fun startRental(rentalId: String): Result<RentalResponse> =
        runCatching { rentalApi.start(rentalId) }
